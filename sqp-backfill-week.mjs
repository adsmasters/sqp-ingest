...
async function rankAndCap(asins){
  ...
  // Manuell angeforderte ASINs (asin_focus am Kunden) laufen IMMER mit
  let focus=[];
  try{
  ...
  if(!sales.size){ console.log(`  Kein Umsatz-Ranking für ${cc} — KEINE Kappung, alle ${ranked.length} ASINs.`); return withFocus(ranked); }
